...
static void free_entity_payload(McEntity *entity){
    free(entity->save_data);
    entity->save_data = NULL;
    entity->save_size = 0;
}
...
void mc_simulation_reset_runtime(McSimulation *simulation, uint64_t seed){
    if(simulation == NULL) return;
    for(size_t i = 0; i < MC_MAX_ENTITIES; i++) free_entity_payload(&simulation->entities[i]);
...
void mc_simulation_reset(McSimulation *simulation, uint64_t seed){
    ...
    mc_simulation_reset_runtime(simulation, seed);
}
...
McEntity *mc_simulation_spawn_with_id(McSimulation *simulation, McEntityKind kind, McTeam team,
                                      McEntityId id, float x, float y){
    if(simulation == NULL || team >= MC_TEAM_COUNT || id == MC_ENTITY_NONE || mc_simulation_find(simulation, id) != NULL){
        return NULL;
    }
    for(size_t i = 0; i < MC_MAX_ENTITIES; i++){
        ...
        if(id >= simulation->next_entity_id && id < UINT32_MAX) simulation->next_entity_id = id + 1;
        return entity;
    }
    return NULL;
}
...
McEntity *mc_simulation_spawn(McSimulation *simulation, McEntityKind kind, McTeam team, float x, float y){
    ...
    McEntityId id = simulation->next_entity_id;
    if(id == MC_ENTITY_NONE) id = 1;
    while(mc_simulation_find(simulation, id) != NULL && id < UINT32_MAX) id++;
    if(id == MC_ENTITY_NONE || mc_simulation_find(simulation, id) != NULL) return NULL;
    McEntity *entity = mc_simulation_spawn_with_id(simulation, kind, team, id, x, y);
    if(entity == NULL) return NULL;
    if(simulation->next_entity_id == id && id < UINT32_MAX) simulation->next_entity_id++;
    return entity;
}

McStatus mc_simulation_attach_entity_data(McEntity *entity, uint8_t class_id, const uint8_t *data, size_t size){
    if(entity == NULL || (data == NULL && size != 0)) return MC_INVALID_ARGUMENT;
    uint8_t *copy = NULL;
    if(size != 0){
        copy = malloc(size);
    ...
    free_entity_payload(entity);
    entity->class_id = class_id;
    entity->save_data = copy;
    entity->save_size = size;
    return MC_OK;
}
...
McStatus mc_simulation_place_block(McSimulation *simulation, McBlockId block, McTeam team, uint16_t x, uint16_t y){
    ...
    if(size == 0 || size > simulation->world.width || size > simulation->world.height ||
       x > simulation->world.width - size || y > simulation->world.height - size) return MC_INVALID_ARGUMENT;
...
uint64_t mc_simulation_hash(const McSimulation *simulation){
    ...
    hash = fnv1a_bytes(hash, &simulation->next_entity_id, sizeof(simulation->next_entity_id));
    ...
    size_t tile_count = (size_t)simulation->world.width * simulation->world.height;
    if(simulation->world.tiles != NULL) hash = fnv1a_bytes(hash, simulation->world.tiles, tile_count * sizeof(*simulation->world.tiles));
    if(simulation->world.tile_data != NULL) hash = fnv1a_bytes(hash, simulation->world.tile_data, tile_count * sizeof(*simulation->world.tile_data));
    for(size_t i = 0; i < MC_TEAM_COUNT; i++) hash = fnv1a_bytes(hash, &simulation->team_inventory[i], sizeof(simulation->team_inventory[i]));
    for(size_t i = 0; i < MC_MAX_ENTITIES; i++){
        ...
        hash = fnv1a_bytes(hash, &entity->rotation, sizeof(entity->rotation));
        hash = fnv1a_bytes(hash, &entity->health, sizeof(entity->health));
        hash = fnv1a_bytes(hash, &entity->lifetime, sizeof(entity->lifetime));
        hash = fnv1a_bytes(hash, &entity->tile_x, sizeof(entity->tile_x));
        hash = fnv1a_bytes(hash, &entity->tile_y, sizeof(entity->tile_y));
        hash = fnv1a_bytes(hash, &entity->block, sizeof(entity->block));
        hash = fnv1a_bytes(hash, &entity->class_id, sizeof(entity->class_id));
        hash = fnv1a_bytes(hash, &entity->save_size, sizeof(entity->save_size));
        if(entity->save_data != NULL) hash = fnv1a_bytes(hash, entity->save_data, entity->save_size);
    }
    return hash;
}
